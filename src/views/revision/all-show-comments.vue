      ...
      <div v-if="docRevisionInfo.length" class="tabs-wrapper">
        <el-tabs type="card" value="wrong">
          <el-tab-pane :label="'错误 (' + wrongCommentList.length + ')'" name="wrong">
            <comment-items
              :comments="wrongCommentList"
              :storeName="scope"
              :tabsValue="'wrong'"
              :readonly="true"></comment-items>
          </el-tab-pane>
          <el-tab-pane :label="'冲突 (' + duplicateCommentList.length + ')'" name="duplicate">
            <comment-items
              :comments="duplicateCommentList"
              :storeName="scope"
              :tabsValue="'duplicate'"
              :readonly="true"></comment-items>
          </el-tab-pane>
          <el-tab-pane :label="'多匹配 (' + multiMatchCommentList.length + ')'" name="multi_match">
            <comment-items
              :comments="multiMatchCommentList"
              :storeName="scope"
              :tabsValue="'multi_match'"
              :readonly="true"></comment-items>
          </el-tab-pane>
          <el-tab-pane :label="'不确定匹配 (' + possibleMatchCommentList.length + ')'" name="possible_match">
            <comment-items
              :comments="possibleMatchCommentList"
              :storeName="scope"
              :tabsValue="'possible_match'"
              :readonly="true"
              no-accept-btn></comment-items>
          </el-tab-pane>
          <el-tab-pane :label="'正确 (' + correctCommentList.length + ')'" name="correct">
            <comment-items
              :comments="correctCommentList"
              :storeName="scope"
              :tabsValue="'correct'"
              :readonly="true"></comment-items>
          </el-tab-pane>
          ...
          <el-tab-pane :label="'其他比例 (' + valueRateCommentList.length + ')'" name="value_rate">
            <comment-items
              :comments="valueRateCommentList"
              :storeName="scope"
              :tabsValue="'value_rate'"
              :readonly="true"></comment-items>
          </el-tab-pane>
        </el-tabs>
      </div>
...
export default {
  ...
  computed: {
    pager () {
      return this.$store.state[scope].pager
    },
    correctCommentList () {
      return this.$store.getters[scope + '/correct_item_list']
    },
    duplicateCommentList () {
      return this.$store.getters[scope + '/duplicate_item_list']
    },
    multiMatchCommentList () {
      return this.$store.getters[scope + '/multi_match_item_list']
    },
    wrongCommentList () {
      return this.$store.getters[scope + '/wrong_item_list']
    },
    noMatchCommentList () {
      return this.$store.getters[scope + '/no_match_item_list']
    },
    possibleMatchCommentList () {
      return this.$store.getters[scope + '/possible_match_item_list']
    },
    valueCommentList () {
      return this.$store.getters[scope + '/value_item_list']
    },
    valueRateCommentList () {
      return this.$store.getters[scope + '/value_rate_item_list']
    },
    docRevisionInfo () {
      return this.$store.state[scope].list
    }
  },
  beforeMount () {
    this.$store.dispatch(scope + '/fetch_all_reject_items').then(() => {
      this.loading = false
    })
  },
  methods: {
    routerGoBack () {
      this.$store.commit(scope + '/set_doc_revision_list', [])
      this.$router.push('/upload/file/list/all')
    },
    handleCurrentPageChange (page) {
      this.$store.commit(scope + '/set_pager', {
        page: page,
