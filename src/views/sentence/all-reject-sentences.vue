<template>
  <div class="container">
    ...
    <div class="file-detail-content layout-content"
      ...
      <h3 class="mb10">所有拒绝的句子</h3>
      <div class="tags-groups clearfloat">
        <div  v-for="tagRow in tags" class="tags-group-wrapper">
          <el-tag
            v-for="item in tagRow"
      ...
      <div class="tabs-wrapper">
        <el-tabs type="card" :value="activeTabName" @tab-click="handleTabClick">
...
export default {
  ...
  beforeMount () {
    ...
    this.$store.dispatch('allSentences/fetch_list', {type: this.activeTabName, status: 'rejected'})
      .then(_ => {
        this.loading = false
      })
      .catch(error => {
  ...
  methods: {
    ...
    handleClickTag (tag) {
      let curTag = this.$store.state.allSentences.tags.filter((item) => {
        return item.cur
      })
      if (curTag.length >= 1) {
        if (tag.id === curTag[0].id) {
          this.$store.commit('allSentences/clear_tag_cur')
        } else {
          this.$store.commit('allSentences/set_tag_cur', tag.id)
        }
      } else {
        this.$store.commit('allSentences/set_tag_cur', tag.id)
      }
      this.loading = true
      this.$store.dispatch('allSentences/fetch_list', {type: this.activeTabName, status: 'rejected', tag: tag})
        .then(_ => {
          this.loading = false
        })
        .catch(error => {
