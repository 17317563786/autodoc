import Vue from 'vue'
import { fetchAllShowComments, getDocHtmlSegment,
...
export default {
  ...
  mutations: {
    [scope + '/set_doc_revision_list'] (state, data) {
      state.list = data
    },
    [scope + '/set_pager'] (state, pager) {
      Vue.set(state, 'pager', pager)
    },
    [scope + '/set_tags'] (state, tags) {
      Vue.set(state, 'tags', tags)
    },
    [scope + '/set_tag_cur'] (state, tagId) {
    ...
    [scope + '/set_item_f_html_paragraph'] (state, {revisionItemId, html}) {
    ...
    [scope + '/set_item_f_dup_html_paragraph'] (state, {revisionItemId, dupParaIndex, html}) {
      state.list.forEach((item) => {
        if (item.id === revisionItemId) {
    ...
    [scope + '/set_item_f_matched_html_paragraph'] (state, {revisionItemId, matchedParaIndex, html}) {
      state.list.forEach((item) => {
        if (item.id === revisionItemId) {
    ...
    [scope + '/set_item_f_html_table'] (state, {revisionItemId, tableIndex, html}) {
      if (html === '' || html === null) {
        return
      }
      state.list.forEach((item) => {
    ...
    [scope + '/set_item_status'] (state, {revisionItemId, status}) {
      state.list.forEach((item) => {
        if (item.id === revisionItemId) {
          if (status === 'approve') {
            item.status = 10
    ...
    [scope + '/set_item_reject_tags'] (state, {revisionItemId, rejectTags}) {
      state.list.forEach((item) => {
        if (item.id === revisionItemId) {
    ...
    [scope + '/del_revision_item'] (state, revisionItemId) {
  ...
  actions: {
    [scope + '/fetch_all_reject_items'] ({ commit, state }) {
      return fetchAllShowComments(state.pager.page, state.pager.size).then((resp) => {
        ...
        commit(scope + '/set_doc_revision_list', resp.data.items)
        commit(scope + '/set_pager', {
          page: resp.data.page,
          size: resp.data.size,
    ...
    [scope + '/fetch_item_html_segment'] ({ commit, state }, { docId, revisionItemId }) {
      let promiseArr = []
      let typeArr = []
    ...
    [scope + '/push_item_revision_status'] ({ commit, state }, { revisionItemId, status, rejectTags }) {
      return new Promise((resolve, reject) => {
        let rejectTagsArr = []
        if (rejectTags) {
        ...
        confirmItemRevisionStatus(revisionItemId, status, rejectTagsArr.join(',')).then((resp) => {
          ...
          commit(scope + '/set_item_status', { revisionItemId, status })
          if (rejectTags) {
            commit(scope + '/set_item_reject_tags', { revisionItemId, rejectTags })
          }
        }).catch((err) => {
          reject(err)
          console.log(err)
    ...
    [scope + '/push_item_reject_tags'] ({ commit, state }, { revisionItemId, rejectTags }) {
      return new Promise((resolve, reject) => {
    ...
    [scope + '/fetch_all_tags'] ({ commit, state }) {
      if (state.tags.length === 0) {
        return fetchAllTags().then((resp) => {
          resp.data.forEach((item) => {
            item.cur = false
          })
          commit(scope + '/set_tags', resp.data)
        })
      } else {
        state.tags.forEach((item) => {
          item.cur = false
    ...
    [scope + '/push_revision_item_issue_status'] ({ commit, state }, { revisionItemId, status }) {
      return pushRevisionItemIssueStatus(revisionItemId, status).then((resp) => {
        commit(scope + '/del_revision_item', revisionItemId)
      })
    }
  },
  getters: {
    [scope + '/correct_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'correct') return true
      })
    },
    [scope + '/duplicate_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'duplicate') return true
      })
    },
    [scope + '/multi_match_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'multi_match') return true
      })
    },
    [scope + '/wrong_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'wrong') return true
      })
    },
    [scope + '/no_match_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'no_match') return true
      })
    },
    [scope + '/possible_match_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'possible_match') return true
      })
    },
    [scope + '/value_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'value') return true
      })
    },
    [scope + '/value_rate_item_list'] (state) {
      return state.list.filter((item) => {
