import Vue from 'vue'
import { fetchAllFixedRejectItems, getDocHtmlSegment, fetchAllTags, revisionItemAddTag } from '../api.js'
...
export default {
  ...
  mutations: {
    ['allFixedRejectItems/set_doc_revision_list'] (state, data) {
      state.list = data
    },
    ['allFixedRejectItems/set_pager'] (state, pager) {
      Vue.set(state, 'pager', pager)
    },
    ['allFixedRejectItems/set_tags'] (state, tags) {
      Vue.set(state, 'tags', tags)
    },
    ['allFixedRejectItems/set_tag_cur'] (state, tagId) {
    ...
    ['allFixedRejectItems/set_item_f_html_paragraph'] (state, {revisionItemId, html}) {
    ...
    ['allFixedRejectItems/set_item_f_dup_html_paragraph'] (state, {revisionItemId, dupParaIndex, html}) {
      state.list.forEach((item) => {
        if (item.id === revisionItemId) {
    ...
    ['allFixedRejectItems/set_item_f_matched_html_paragraph'] (state, {revisionItemId, matchedParaIndex, html}) {
      state.list.forEach((item) => {
        if (item.id === revisionItemId) {
    ...
    ['allFixedRejectItems/set_item_f_html_table'] (state, {revisionItemId, tableIndex, html}) {
      if (html === '' || html === null) {
        return
      }
      state.list.forEach((item) => {
    ...
    ['allFixedRejectItems/del_revision_item'] (state, {revisionItemId}) {
  ...
  actions: {
    ['allFixedRejectItems/fetch_all_fixed_reject_items'] ({ commit, state }, tagName) {
      return fetchAllFixedRejectItems(state.pager.page, state.pager.size, tagName).then((resp) => {
        ...
        commit('allFixedRejectItems/set_doc_revision_list', resp.data.items)
        commit('allFixedRejectItems/set_pager', {
          page: resp.data.page,
          size: resp.data.size,
    ...
    ['allFixedRejectItems/fetch_item_html_segment'] ({ commit, state }, { docId, revisionItemId }) {
      let promiseArr = []
      let typeArr = []
          ...
          result.forEach((item, index) => {
            if (item.data.html !== '' && item.data.html !== null) {
              if (typeArr[index].type === 'paragraph') {
                commit('allFixedRejectItems/set_item_f_html_paragraph', {
                  revisionItemId,
                  html: item.data.html
                })
              } else if (typeArr[index].type === 'table') {
                commit('allFixedRejectItems/set_item_f_html_table', {
                  revisionItemId,
                  tableIndex: typeArr[index].index,
                  html: item.data.html
                })
              } else if (typeArr[index].type === 'dup_paragraph') {
                commit('allFixedRejectItems/set_item_f_dup_html_paragraph', {
                  revisionItemId,
                  dupParaIndex: typeArr[index].index,
                  html: item.data.html
                })
              } else if (typeArr[index].type === 'matched_paragraph') {
                commit('allFixedRejectItems/set_item_f_matched_html_paragraph', {
                  revisionItemId,
    ...
    ['allFixedRejectItems/push_item_reject_tags'] ({ commit, state }, { revisionItemId, rejectTags }) {
      return new Promise((resolve, reject) => {
        revisionItemAddTag(revisionItemId, rejectTags.map(item => item.name).join(',')).then((resp) => {
          ...
          commit('allFixedRejectItems/set_item_reject_tags', { revisionItemId, rejectTags })
        }).catch((err) => {
          reject(err)
          console.log(err)
        })
    ...
    ['allFixedRejectItems/fetch_all_tags'] ({ commit, state }) {
      if (state.tags.length === 0) {
        return fetchAllTags().then((resp) => {
          resp.data.forEach((item) => {
            item.cur = false
          })
          commit('allFixedRejectItems/set_tags', resp.data)
        })
      } else {
  ...
  getters: {
    ['allFixedRejectItems/correct_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'correct') return true
      })
    },
    ['allFixedRejectItems/duplicate_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'duplicate') return true
      })
    },
    ['allFixedRejectItems/multi_match_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'multi_match') return true
      })
    },
    ['allFixedRejectItems/wrong_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'wrong') return true
      })
    },
    ['allFixedRejectItems/no_match_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'no_match') return true
      })
    },
    ['allFixedRejectItems/possible_match_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'possible_match') return true
      })
    },
    ['allFixedRejectItems/value_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'value') return true
      })
    },
    ['allFixedRejectItems/value_rate_item_list'] (state) {
      return state.list.filter((item) => {
