import Vue from 'vue'
import { fetchAllAcceptItems, confirmItemRevisionStatus, getDocHtmlSegment, revisionItemAddTag } from '../api.js'
...
export default {
  ...
  mutations: {
    ['allAcceptItems/set_doc_revision_list'] (state, data) {
      state.list = data
    },
    ['allAcceptItems/set_pager'] (state, pager) {
      Vue.set(state, 'pager', pager)
    },
    ['allAcceptItems/set_item_f_html_paragraph'] (state, {revisionItemId, html}) {
    ...
    ['allAcceptItems/set_item_f_dup_html_paragraph'] (state, {revisionItemId, dupParaIndex, html}) {
      state.list.forEach((item) => {
        if (item.id === revisionItemId) {
    ...
    ['allAcceptItems/set_item_f_matched_html_paragraph'] (state, {revisionItemId, matchedParaIndex, html}) {
      state.list.forEach((item) => {
        if (item.id === revisionItemId) {
    ...
    ['allAcceptItems/set_item_f_html_table'] (state, {revisionItemId, tableIndex, html}) {
      if (html === '' || html === null) {
        return
      }
      state.list.forEach((item) => {
    ...
    ['allAcceptItems/set_item_status'] (state, {revisionItemId, status}) {
      state.list.forEach((item) => {
        if (item.id === revisionItemId) {
          if (status === 'approve') {
            item.status = 10
    ...
    ['allAcceptItems/set_item_reject_tags'] (state, {revisionItemId, rejectTags}) {
      state.list.forEach((item) => {
        if (item.id === revisionItemId) {
    ...
    ['allAcceptItems/del_revision_item'] (state, {revisionItemId}) {
  ...
  actions: {
    ['allAcceptItems/fetch_all_accept_items'] ({ commit, state }) {
      return fetchAllAcceptItems(state.pager.page, state.pager.size).then((resp) => {
        ...
        commit('allAcceptItems/set_doc_revision_list', resp.data.items)
        commit('allAcceptItems/set_pager', {
          page: resp.data.page,
          size: resp.data.size,
    ...
    ['allAcceptItems/fetch_item_html_segment'] ({ commit, state }, { docId, revisionItemId }) {
      let promiseArr = []
      let typeArr = []
          ...
          result.forEach((item, index) => {
            if (item.data.html !== '' && item.data.html !== null) {
              if (typeArr[index].type === 'paragraph') {
                commit('allAcceptItems/set_item_f_html_paragraph', {
                  revisionItemId,
                  html: item.data.html
                })
              } else if (typeArr[index].type === 'table') {
                commit('allAcceptItems/set_item_f_html_table', {
                  revisionItemId,
                  tableIndex: typeArr[index].index,
                  html: item.data.html
                })
              } else if (typeArr[index].type === 'dup_paragraph') {
                commit('allAcceptItems/set_item_f_dup_html_paragraph', {
                  revisionItemId,
                  dupParaIndex: typeArr[index].index,
                  html: item.data.html
                })
              } else if (typeArr[index].type === 'matched_paragraph') {
                commit('allAcceptItems/set_item_f_matched_html_paragraph', {
                  revisionItemId,
    ...
    ['allAcceptItems/push_item_revision_status'] ({ commit, state }, { revisionItemId, status, rejectTags }) {
      return new Promise((resolve, reject) => {
        let rejectTagsArr = []
        if (rejectTags) {
        ...
        confirmItemRevisionStatus(revisionItemId, status, rejectTagsArr.join(',')).then((resp) => {
          ...
          commit('allAcceptItems/set_item_status', { revisionItemId, status })
          if (rejectTags) {
            commit('allAcceptItems/set_item_reject_tags', { revisionItemId, rejectTags })
          }
        }).catch((err) => {
          reject(err)
          console.log(err)
    ...
    ['allAcceptItems/push_item_reject_tags'] ({ commit, state }, { revisionItemId, rejectTags }) {
      return new Promise((resolve, reject) => {
  ...
  getters: {
    ['allAcceptItems/correct_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'correct') return true
      })
    },
    ['allAcceptItems/duplicate_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'duplicate') return true
      })
    },
    ['allAcceptItems/multi_match_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'multi_match') return true
      })
    },
    ['allAcceptItems/wrong_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'wrong') return true
      })
    },
    ['allAcceptItems/no_match_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'no_match') return true
      })
    },
    ['allAcceptItems/possible_match_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'possible_match') return true
      })
    },
    ['allAcceptItems/value_item_list'] (state) {
      return state.list.filter((item) => {
        if (item.type === 'value') return true
      })
    },
    ['allAcceptItems/value_rate_item_list'] (state) {
      return state.list.filter((item) => {
