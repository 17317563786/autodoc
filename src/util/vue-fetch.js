...
module.exports.install = function (Vue, initOptions = {}) {
  ...
  let vueFetch = {
    ...
    start (options) {
      ...
      if (options.cache) {
        return this.getDataFromCache(options)
      } else {
        return this.fetchData(options)
      }
    },
    getDataFromCache (options) {
      let url = options.url
      let type = options.type
      if (options.cacheType === 'sessionStorage') {
        if (window.sessionStorage[type + ':' + url]) {
          return Promise.resolve(JSON.parse(window.sessionStorage[type + ':' + url]))
        } else {
          return this.fetchData(options).then((resp) => {
            if (resp.status) {
              window.sessionStorage[type + ':' + url] = JSON.stringify(resp)
            }
            return resp
          })
        }
